...
def test_classificador_com_dados_do_anexo():
    """
    Testa a acurácia do classificador de sentimento com os exemplos fornecidos no anexo do teste técnico.

    Para cada exemplo, compara a classificação esperada com a detectada. O teste falha caso menos de 50% das avaliações
    sejam classificadas corretamente, indicando que o modelo atual não é adequado para o domínio proposto.

    Prints:
        Lista de erros com nome do cliente, classificação esperada e detectada.

    Raises:
        AssertionError: Se a taxa de acertos for inferior a 50%.
    """
    erros = []
    ...
    assert (
        acertos >= len(exemplos) * 0.5
    ), "Classificador falhou em mais de 50% dos exemplos do anexo"
